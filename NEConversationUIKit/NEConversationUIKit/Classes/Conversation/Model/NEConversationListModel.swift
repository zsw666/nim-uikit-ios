...
enum ConversationListSort {
  static func comesBefore(_ lhsModel: NEConversationListModel,
                          _ rhsModel: NEConversationListModel) -> Bool {
    (lhsModel.conversation?.sortOrder ?? 0) > (rhsModel.conversation?.sortOrder ?? 0)
  }
}
